...
if(count($arResult["PERSON_TYPE"]) > 1)
{
	?>
	<div class="section">
		<div class="h3"><?=GetMessage("SOA_TEMPL_PERSON_TYPE")?></div>
		<?foreach($arResult["PERSON_TYPE"] as $v):?>
			<div class="label fl-left">
				<input type="radio" id="PERSON_TYPE_<?=$v["ID"]?>" name="PERSON_TYPE" value="<?=$v["ID"]?>"<?if ($v["CHECKED"]=="Y") echo " checked=\"checked\"";?> onClick="submitForm()"> <label for="PERSON_TYPE_<?=$v["ID"]?>">
					<?=$v["NAME"]?></label>
			</div>
			<?endforeach;?>
		<div class="clear"></div>
