...
<div class="bx_ordercart">
	<div class="h3 ordercart-title"><?=GetMessage("SALE_PRODUCTS_SUMMARY");?></div>
	<div class="bx_ordercart_order_table_container">
		<table class="table-reset tbi">
			<thead class="tbi-thead">
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arColumn):
						...
						<?
							echo $arColumn["name"];
                        elseif (strpos($arColumn["id"], 'PROPERTY_') === 0):
                            $arProperties[$arColumn["id"]] = $arColumn["name"];
						else:
						?>
				...
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arColumn):
							...
							<td class="tbi-td item" >
								...
								<div class="bx_ordercart_itemart">
                                    ...
                                    foreach ($arProperties as $key => $val):
                                        if(!empty($arItem[$key])){
                                            echo $val.":&nbsp;<span>".$arItem[$key]."<span>".(in_array($key, array('PROPERTY_WIDTH_VALUE', 'PROPERTY_LONG_VALUE', 'PROPERTY_HEIGHT_VALUE')) ? GetMessage("B_MM") : '')."<br/>";
                                        }
										endforeach;
									endif;
									?>
							...
							<td class="tbi-td custom right">
								<div class="total_price">
									<?if($arColumn["id"] == "DISCOUNT_PRICE_PERCENT_FORMATED"):?>
										<?if(intval($arResult["SALE_PRICE"][$arItem["PRODUCT_ID"]])>$arItem["PRICE"] && $arResult["SALE_PRICE"][$arItem["PRODUCT_ID"]]):?>
											<?=round($arItem["PRICE"]/$arResult["SALE_PRICE"][$arItem["PRODUCT_ID"]]*100-100)?>&nbsp;%
										<?endif;?>
									<?else:?>
										<?=$arItem[$arColumn["id"]]?></div>
									<?endif;?>

							</td>
						<?
                        elseif (!empty($arProperties) && array_key_exists($arColumn["id"], $arProperties)):
                            continue;
                        ?>
						<?else: // some property value
							...
							?>
							<td class="tbi-td custom" style="<?=$columnStyle?>">
								<?
								foreach ($arItem[$arColumn["id"]] as $arValues):
									if ($arValues["type"] == "image"):
									?>
							...
							?>
							<td class=" tbi-tdcustom" style="<?=$columnStyle?>">
								<?
	...
	<div class="bx_ordercart_order_pay">
		...
		<div class="bx_ordercart_order_pay_left">
			<div class="h3">Комментарии к заказу:</div>
			<div class="bx_block w100">
				<textarea id="ORDER_DESCRIPTION" name="ORDER_DESCRIPTION" style="max-width: 100%; min-height: 78px; width: 807px; height: 83px;"></textarea>
			</div>
