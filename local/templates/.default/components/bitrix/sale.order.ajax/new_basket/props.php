...
<h4><?=GetMessage("SOA_TEMPL_PROP_INFO")?></h4>
	...
	if (is_array($arResult["ORDER_PROP"]["USER_PROFILES"]) && !empty($arResult["ORDER_PROP"]["USER_PROFILES"])):
		if ($arParams["ALLOW_NEW_PROFILE"] == "N"):
		?>
			...
			<div class="bx_block r3x1">
					...
					if (count($arResult["ORDER_PROP"]["USER_PROFILES"]) == 1)
					{
						foreach($arResult["ORDER_PROP"]["USER_PROFILES"] as $arUserProfiles)
						{
							...
							?>
							<input class="form-control" class="form-control" type="hidden" name="PROFILE_ID" id="ID_PROFILE_ID" value="<?=$arUserProfiles["ID"]?>" />
							<?
						}
					}
					else
...
<div class="bx_section props">
	<div class="h3">
		...
		?>
		<input class="form-control" type="hidden" name="showProps" id="showProps" value="<?=($_POST["showProps"] == 'Y' ? 'Y' : 'N')?>" />
	</div>
