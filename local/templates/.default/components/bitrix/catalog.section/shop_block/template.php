...
if(count($arResult["ITEMS"]) < 1) return false;

?>
<?if($arParams['ELEMENT_TEMPLATE'] == "Y" && strlen($arParams["DETAIL_TITLE_RECOMMEND"]) > 0):?>
    <div class="h2"><?=$arParams["DETAIL_TITLE_RECOMMEND"]?></div>
<?endif;?>
<?if($arParams["DISPLAY_TOP_PAGER"]):?>
    <?=$arResult["NAV_STRING"]?>
<?endif;?>
<div class="catalog-block catalog-list js-height">
    <div class="row row-clear">
        <?$i=1;foreach($arResult["ITEMS"] as $arItem):
            $this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_EDIT"));
            $this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_DELETE"), array("CONFIRM" => GetMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')));
            ?>
            <div class="item col-sm-4 col-12 catalog-item item-box" data-id="<?=$arItem['ID']?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
                <div class="wrap">
