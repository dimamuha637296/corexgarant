...
if($arParams["DISPLAY_PICTURE"]!="N"){
    ...
    $arParams["LIST_IMG_WIDTH"] = intval($arParams["LIST_IMG_WIDTH"]) > 0 ? intval($arParams["LIST_IMG_WIDTH"]) : '275';
    $arParams["LIST_IMG_HEIGHT"] =  intval($arParams["LIST_IMG_HEIGHT"]) > 0 ? intval($arParams["LIST_IMG_HEIGHT"]) : '230';
    ...
    foreach ($arResult['ITEMS'] as $key => $arElement)
    {
        if(intval($arElement['~DETAIL_PICTURE']) > 0)
        {
            $arFileTmp_small = CFile::ResizeImageGet(
                ...
                array("width" => $arParams["LIST_IMG_WIDTH"], "height" => $arParams["LIST_IMG_HEIGHT"]),
