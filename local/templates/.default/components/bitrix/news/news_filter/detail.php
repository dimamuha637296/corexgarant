...
if(strlen($arResult["VARIABLES"]["SECTION_CODE"]) > 0){
	...
	if($obCache->InitCache($life_time, $cache_id, "/")) :
		...
		$rsSect = CIBlockSection::GetList(array('sort' => 'asc'),$arFilter, false, array('ID', 'IBLOCK_ID'), array('nPageSize' => 1));
		while ($arSect = $rsSect->GetNext())
		{
			$arResult["SECTION_INFO"]["CUR_SECTION"] = $arSect;
		}
...
if($SECTIONS["ID"] != $SECTIONS["CUR_SECTION"] ["ID"]){
	$addSectionChain = "Y";
}else{
$addSectionChain = "N";
}
...
<?$ElementID = $APPLICATION->IncludeComponent(
	...
	"",
	Array(
		"ADD_SECTIONS_CHAIN" => $addSectionChain ,
